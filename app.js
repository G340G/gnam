...
function remoteQty(n,measure){
 ...
 const m=String(measure||'').toLowerCase().replace(/–/g,'-').trim();
 const meta=canonMeta(cn);
 if(!m || /to taste|as needed|q\.?b\.?|for serving|for garnish|divided/.test(m)) return null;
 let q=null;
 const frac=m.match(/(\d+(?:[\.,]\d+)?|\d+\s+\/\s+\d+)/);
 if(frac){
   const raw=frac[1].replace(',','.');
   q=raw.includes('/')?(()=>{const z=raw.split(/\s+\/\s+/);return Number(z[0])/Number(z[1])})():Number(raw);
 }
 ...
 else if(/lb|pound/.test(m))q*=453.592;
 else if(/oz|ounce/.test(m))q*=28.35;
 else if(/cup/.test(m))q*=({grain:75,vegetable:120,fruit:140,dairy:200,legume:170,meat:140,fish:140}[meta.group]||120);
 else if(/tablespoon|tbsp/.test(m))q*=({oil:13,dairy:15}[meta.group]||12);
 else if(/teaspoon|tsp/.test(m))q*=4;
 else if(/ml|milliliter/.test(m))return [Math.max(.1,q),'ml'];
 else if(/g|gram/.test(m))return [Math.max(.1,q),'g'];
 if(meta.unit==='pz')return [Math.max(.1,q),'pz'];
 // Reject ambiguous package/count units rather than inventing grams.
 if(/can|tin|bunch|packet|pack|bag|jar|clove|stalk|sprig|handful|slice|fillet|large|medium|small/.test(m))return null;
 return [Math.max(.1,q),meta.unit==='ml'?'ml':'g'];
}
...
function remoteCovered(r,pantry){return r.remoteIntegrity && recipeIngredients(r).every(x=>availableFor(x.name,pantry)>=baseAmount(x.qty,x.unit)-0.01)}
function qaRecipe(r){
...
async function buildPlan(){
 ...
 let remotePool=[];
 if(window.GNAM_REMOTE){try{remotePool=(await window.GNAM_REMOTE.getLiveRecipePool(source)).map(normalizeRemoteRecipe).filter(Boolean).filter(r=>r.remoteIntegrity).filter(qaRecipe).filter(r=>Number(r.diff||2)<=Number(state.difficulty||2));}catch{remotePool=[]}}
